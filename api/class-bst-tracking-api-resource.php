<?php
/**
 * @package     blazing-wooCommerce-tracking/API
 * @category    API
 * @since       1.0
 *
...
class BS_Shipment_Tracking_API_Resource
{
	 ...
	 */
	public function __construct(BS_Shipment_Tracking_API_Server $server)
	{

		$this->server = $server;

		// automatically register routes for sub-classes
		add_filter('bst_api_endpoints', array($this, 'register_routes'));
		...
		foreach (array('order', 'coupon', 'customer', 'product', 'report') as $resource) {
			...
			add_filter("bst_api_{$resource}_response", array($this, 'maybe_add_meta'), 15, 2);
			add_filter("bst_api_{$resource}_response", array($this, 'filter_response_fields'), 20, 3);
		}
	}
	...
	protected function validate_request($id, $type, $context)
	{
		...
		// validate ID
		if (empty($id))
			return new WP_Error("bst_api_invalid_{$resource_name}_id", sprintf(__('Invalid %s ID', 'bs_ship_track'), $type), array('status' => 404));
		...
		if ('customer' !== $type) {
			...
			// validate post type
			if ($type !== $post_type)
				return new WP_Error("bst_api_invalid_{$resource_name}", sprintf(__('Invalid %s', 'bs_ship_track'), $resource_name), array('status' => 404));
			...
			switch ($context) {
				...
				case 'read':
					if (!$this->is_readable($post))
						return new WP_Error("bst_api_user_cannot_read_{$resource_name}", sprintf(__('You do not have permission to read this %s', 'bs_ship_track'), $resource_name), array('status' => 401));
					break;

				case 'edit':
					if (!$this->is_editable($post))
						return new WP_Error("bst_api_user_cannot_edit_{$resource_name}", sprintf(__('You do not have permission to edit this %s', 'bs_ship_track'), $resource_name), array('status' => 401));
					break;

				case 'delete':
					if (!$this->is_deletable($post))
						return new WP_Error("bst_api_user_cannot_delete_{$resource_name}", sprintf(__('You do not have permission to delete this %s', 'bs_ship_track'), $resource_name), array('status' => 401));
					break;
			}
		}
	...
	protected function delete($id, $type, $force = false)
	{
		...
		if ('customer' === $type) {
			...
			if ($result)
				return array('message' => __('Permanently deleted customer', 'bs_ship_track'));
			else
				return new WP_Error('bst_api_cannot_delete_customer', __('The customer cannot be deleted', 'bs_ship_track'), array('status' => 500));
			...
			if (!$result)
				return new WP_Error("bst_api_cannot_delete_{$resource_name}", sprintf(__('This %s cannot be deleted', 'bs_ship_track'), $resource_name), array('status' => 500));

			if ($force) {
				return array('message' => sprintf(__('Permanently deleted %s', 'bs_ship_track'), $resource_name));

			} else {

				$this->server->send_status('202');

				return array('message' => sprintf(__('Deleted %s', 'bs_ship_track'), $resource_name));
			}
		}
	}
